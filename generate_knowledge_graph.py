...
#Load the LLM
llm = Ollama(temperature=0, model="gemma3")
graph_transformer = LLMGraphTransformer(llm=llm)

#Extract graph data from the input text
async def extract_graph_data(text):
    ...
    Returns:
        list: A list of GraphDocument objects contianing nodes and relationships.
    """
...
def visualize_graph(graph_documents):
    ...
    #Create network
    net = Network(height="1200px", width="100%", directed=True, notebook=False, bgcolor="#222222", font_color="white", filter_menu=True, cdn_resources='remote')
   ...
   # Configure graph layout and physics
    ...
    try:
        ...
        print(f"Graph saved tp {os.path.abspath(output_file)}")
        return net
...
def generate_knowledge_graph(text):
    """
    Generates and visualizes a knowledge graph from the input text.

    This function runs the graph extraction asynchronously and the visualizes the resulting graph using pyvis.
